...
try:
    ...
    driver.save_screenshot('D:\iTAP\Recorded_Scenarios\\recordernew\\1\\page1.png')
        ...
        try:
            ...
            break
             ...
             if len(possible_xpaths)-1 == idx:
                ...
                raise Exception('Element Not Found')
    driver.save_screenshot('D:\iTAP\Recorded_Scenarios\\recordernew\\1\\page2.png')
    ...
    for idx,i in enumerate(possible_xpaths):
        try:
            element = WebDriverWait(driver, delay).until(EC.element_to_be_clickable((By.XPATH, i)))
            driver.save_screenshot('D:\iTAP\Recorded_Scenarios\\recordernew\\1\\page3.png')
    ...
    for idx,i in enumerate(possible_xpaths):
        try:
            element = WebDriverWait(driver, delay).until(EC.element_to_be_clickable((By.XPATH, i)))
            driver.save_screenshot('D:\iTAP\Recorded_Scenarios\\recordernew\\1\\page4.png')
            output[i] = element.text
            actions = ActionChains(driver)
            actions.move_to_element(element).perform()
            try:
    ...
    for idx,i in enumerate(possible_xpaths):
        try:
            element = WebDriverWait(driver, delay).until(EC.element_to_be_clickable((By.XPATH, i)))
            driver.save_screenshot('D:\iTAP\Recorded_Scenarios\\recordernew\\1\\page5.png')
            output[i] = element.text
            actions = ActionChains(driver)
            actions.move_to_element(element).perform()
            try:
    ...
    for idx,i in enumerate(possible_xpaths):
        try:
            element = WebDriverWait(driver, delay).until(EC.element_to_be_clickable((By.XPATH, i)))
            driver.save_screenshot('D:\iTAP\Recorded_Scenarios\\recordernew\\1\\page6.png')
            output[i] = element.text
            actions = ActionChains(driver)
            actions.move_to_element(element).perform()
            try:
    ...
    with open(r"D:\iTAP\Recorded_Scenarios\recordernew\1\logfile.log", "w+") as external_file:
    ...
    print("test passed - no errors", steps)
    driver.close()
    ...
    path = open(r"D:\iTAP\Recorded_Scenarios\recordernew\1\logfile.log", "w+")
    path.write("test case failed:"+ str(e) + '\n' + steps +  "\n"+message+ "\n Execution stopped at : " +lines + "\nxpath list " + str(xpath_list) + "\nvalue list " + str(value_list) +"\nclass name list " + str(classname_list) +"\ntext list " + str(text_list)  +" \n total time taken : "+total)
    print("test case failed: ", steps)
    path.close()
    driver.close()
